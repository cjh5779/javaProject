...
public class HashMapWordCount {
	public static void main(String[] args) {
			...
			for(int j=0; j<textArray[i].length; j++) {
				if(!textArray[i][j].isEmpty()) {
					String word = textArray[i][j] // 한 단어 추출해서
							.replace("[^a-zA-Z]", "") // 영문자 제외한 나머지 문자 제거
					...
					// 기존값 반환받아 1증가 후 update, 키가 없는 경우 0을 반환받아 1 증가후 put
		...
		// HashMap.value() : collection(V) 타입으로 반환
		// stream() : 배열, 컬렉션 등의 데이터를 람다식 활용 함수형으로 처리하게 해주는 도구
		// 중간 연산은 stream 반환 최종연산은 기본값 또는 객체로 결과 반환
		// HashMap/values() -> stream 처리 -> Integer 객체로 반환
		// mapToint(Integer::inValue) : 기본형 int로 변환 처리
		...
		// 단어 빈도에 따른 통계 출력 : 전체 단어수 : value의 합 / 고유 단어수 : key의 수 -> size()는 entrySet의 수 반환
