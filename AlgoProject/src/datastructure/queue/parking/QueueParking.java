...
public class QueueParking {
    ...
    public static void parkingMenu() {
    	...
    	MENU:while(true) {
    		...
    		System.out.println("0. 주자여부 확인 1. 주차 2. 출차 3. 만차 4. 종료");
    		...
    		case "1" :
    			...
    			if(parking.contains(Character.toLowerCase(Char)) == -1) {
    				if(parking.enqueue(Character.toLowerCase(Char))) {
    		...
    		case "2" :
    			 ...
    			 * 1. 바로 출차 가능한지 확인
    			 * 2. 출차 불가 메시지
    			 *   2-1. 주차 시간 1시간 이상인지 여부
    			 *   2-2. 1시간 미만 : 출차 불가
    			 *   2-3. 1시간 이상 : 출차 - 선행 차량 dequeue 후 다시 enqueue가 모두 완료되면
    			 *   2-4. 출차 요구 차량 출차(2-3 완료 되지 않을 시 출차 불가)
    			 */
    					...
    					if(time >= 1) { // 선행차량 이동 후 출차
    						...
    						if(num1 >= 0) {
    							for(int i=1; i<=num1; i++) {
    								if(parking.isFull()) {
    									System.out.println("주차장에 이동 공간이 없어 선행 차량 이동이 불가능하여 미리 출근");
    									continue MENU;
    								} else {
