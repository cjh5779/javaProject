...
public class Heap<E> {
	...
	/////////////////////////////////
	/// heap에 자료 저장 -> add -> shiftUp(size, target(저장할 값))
	/////////////////////////////////

	public void add(E value) {
		// 배열이 꽉 차있을 경우 현재 크기의 두배로 확장
	...
	private void shiftUpComparable(int idx, E target) {
		// Comparable 사용을 위해 기준 객체 타입을 Comparable 타입으로 형변환
		Comparable<? super E> comp = (Comparable) target;
	...
	private void shiftDownComparator(int idx, E target, Comparator<? super E> comp) {
		...
		/*
		 * 삭제 후에는 배열 사이즈가 기본 크기인 10보다는 크고 요소 개수가 전체 배열 크기의 1/4알 경우
		 * 배열 사이즈를 반으로 줄임
		 */
