...
public class StringMethod {
	public static void main(String[] args) {
		// CharAt : 문자열의 특정 위치(index)의 문자를 반환
		...
		// getByte() : 문자열을 byte 배열로 반환
		// String.length() : 문자열 길이 (문자개수) 반환 반환(공백문자도 하나의 문자로 포함됨)
		// Array.length() : 배열 원소수 반환
		// 문자열을 byte 배열로 변환했을 때 String.length() 와 Array.length()는 같을수도 있고 다를수도 있음
		String str = "안녕하세요";
		...
		// indexOf("시작위치를 알고자하는 문자열") : 문자열이 객체에 포함되어져 있으면 시각 index 반환, 포함되어 있지 않으면 -1반환
		...
		// replace가 적용된 문자열의 원본인 변환되지는 않음, 문자열에서 원본 찾아서 대치 후에 새로운 문자열로 반환
		String oldStr = "자바는 객체지향 언어 입니다. 자바는 풍부한 API를 지원합니다";
		String newStr = oldStr.replace("자바", "JAVA"); // 찾아지는 모든 원본을 찾아서 모두 대치함
		System.out.println(oldStr);
		System.out.println(newStr);
		
		// substring(int beginIndex, int endIndex) : 문자열에서 beginIndex부터 endIndex-1 까지의 부분 문자열 반환
		...
		System.out.println(str3.toLowerCase().equals(str4.toLowerCase())); // true
		System.out.println(str3.toUpperCase().equals(str4.toUpperCase())); // true
		...
		// split("구분문자열") : 문자열내에 구분 문자열을 찾아 분리 후 배열에 저장 후 반환
