...
public class PolyMain {
	public static void main(String[] args) {
		...
		// 다형성
		a = new Cat(); // Animal 타입 참조변수에 Cat 타입의 인스턴스가 대입 Cat -> Animal 자동형변환
		System.out.println("Cat타입 인스턴스가 Animal 참조변수로 대입되면서 형변환 발생");
		a.show();
		
		// 다형성이 진행된 경우 부모클래스 참조 변수는 부모 클래스의 멤버만 접근할 수 있지만
		// 메소드가 재정의된 경우 자식클래스 메소드를 사용하게 됨
		a = new Cat2(); // Cat2는 show가 재정의 되지 않았으므로 Animal 메소드가 호출됨
		a.show(); 
		//a.show1(); // Cat2 인스턴스가 대입이 되었지만 Cat2 인스턴스의 일반 멤버는 사용불가능 재정의 되지 않았다면 부모 클래스 멤버
		...
		// 자동형변환 예시
		ap = new Cat(); // sub클래스 인스턴스 타입이 대입되면서 superclass 타입으로 자동형변환
		ap.show();
		ap.sound();
		
		ap1 = new Dog(); // sub클래스 인스턴스 타입이 대입되면서 superclass 타입으로 자동형변환
		ap1.show();
		ap1.sound();
		ap1.dogMethod(); //sub클래스 메소드 사용 불가능(재정의 되지 않았음)
		ap1.animalMethod(); // super 클래스 메소드 사용 가능
		
		// Dog 객체를 참조하는 변수는 ap1만 있다고 가정 - Dog 객체의 일반 메소드는 사용 불가능
		// 단 api의 참조 주소는 Dog 객체 주소이므로 강제 형변환 가능
		Dog d1 = (Dog)ap1;
		// d1과 ap1은 같은 인스턴스 참조
		d1.dogMethod(); // 자식 객체의 메소드 사용 가능
