...
public class CRUDTestInsertInput {
	public static void main(String[] args) {
		...
		try {
			...
			// statement 객체는 매번 전처리(형식검사를 진행 함 - 완성된 쿼리 구문을 전달해야함)
			...
			// Statement 객체의 하위 인터페이스, 형식검사를 미리 진행함(컴파일러) - 쿼리 완성 후 형식검사 진행하지 않음
			// 변수 바인딩 진행 : ? 바인딩문자 -> 플레이스 홀더 사용, 인수가 많은 쿼리에 적합
			// 처음 한번 전처리 후 다음 사용에는 전처리 하지 않는다
