...
public class DataType {
	public static void main(String[] args) {
		// 정수 타입 : 기본 타입 int, long 형은 기본타입이 아니기 때문에 리터럴 뒤에 l/L 추가해야 함
		
		// 100은 어떤 타입이던 표현가능한 리터럴이므로 long에 넣을 때 l/L 없어도 됨
		int four = 100; // 4byte
		long fourD = 100; // 8byte
		
		// 1000100010001000 4byte로도 표현 불가능한 가중치의 정수임
		// 4byte 표현하지 말고 8byte로 처리하도록 l/L 추가해야 됨
		long longValue = 1000100010001000L;
		...
		// double 이 기본타입이므로 리터럴은 8byte로 처리됨
		double avg = 88.5; // 8byte 리터럴을 8byte 변수에 대입 (동일크기 이므로 문제 없음)
		// float avg1 = 88.5; 8byte 리터럴을 4byte 변수에 대입 (큰 타입을 작은 타입에 넣어서 문제가 생김)
		
		float avg2 = 88.5f; // 실수 리터럴 8byte가 아닌 4byte로 처리 후 4byte 변수에 대입 (오류 없이 처리)
